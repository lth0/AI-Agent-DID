...
第一里程碑明确命名为：**三方案协议与适配器**。该里程碑只在步骤 007 至步骤 039 的代码、链上解析接线、在线入口接线与测试全部通过后完成。当前已完成三方案核心、本地 Hardhat DID 注册与真实解析、统一入口、本地完整 63 项执行，以及 Sepolia 全量入口与严格预检的代码实现；在线入口和真实付费 Sepolia 63 项验收仍待后续步骤完成。
...
| Sepolia DID 链上注册与解析 | 代码已实现并通过自动化测试；未做付费全量验收 | 严格检查 chain ID、Registry 字节码/接口、owner/delegate、确认数与解析结果；任何失败均关闭式返回且不回退 Hardhat |
| Sepolia 全量 63 项执行路径 | 代码已实现并通过自动化测试；未做付费全量验收 | 父进程一次预检，63 个子实验串行执行；仅 H00、L01–L14 共 15 个 Lineage 实例上链；检查余额、latest/pending nonce、治理、epoch、15 个预算命名空间与成本上限；预计 160–164 笔测试网交易 |
| 在线 /auth、/probe、/context_hash 接线 | 待完成 | 当前三方案核心尚未完整接入在线演示入口 |
| 根目录统一 `main.py` 入口 | 已完成并复核 | 支持 `single`、`all`、`list`；方案全名/短名和大小写 case 均统一校验 |
| run_all 与 63 个独立子进程 | 延迟 materialize 修复后已完成并复核 | `sepolia-impl-local-final-a31f6d`：一次 Hardhat 部署、63 个串行独立子进程、63/63 COMPLETED、0 INFRA_ERROR，合计 164 笔交易 |
| 63 个独立 Merkle 根与链上锚定 | 延迟 materialize 修复后已完成并复核 | `sepolia-impl-local-final-a31f6d`：63 个唯一锚定；101 笔 DID+Lineage canonical 交易和 63 个 anchor 全部反查通过 |
| 第一里程碑“三方案协议与适配器” | **尚未完整验收** | 三方案核心、本地链与 Sepolia 严格链路已接线；在线入口仍待完成，且尚无一次真实付费 Sepolia 63 项验收结果 |
...
实跑共完成 27/27 个实例，0 `INFRA_ERROR`，27/27 个锚定可验证，响应符合率为 27/27。Baseline 通过后的链上 materialize/设置阶段共产生 59 笔 Lineage 交易和 59 个事件，Lineage Gas 为 11,737,703，审计锚定 Gas 为 620,880；L09 单项注册两条完整分支并产生 10 笔交易，L12 单项产生 7 笔交易且包含匹配 persistent DID 的 `StatusRevoked(kind=3)`。全部 Lineage 目标拒绝都发生在 Invocation 开始前。
...
`run-id` 必须是单个安全路径组件，不能包含斜杠、反斜杠或路径穿越符号；独立 Hardhat single 必须使用新的 run-id，避免把旧链的 DID setup 误用于新链。完整 Sepolia 入口现已开放：父进程先执行一次完整只读预检，全部通过后才进行共享 DID 初始化并串行执行 63 个子实验；预检或运行中任一基础设施错误都会失败关闭，绝不回退 Hardhat。该路径预计发送 160–164 笔测试网交易，只有用户显式执行并取得完整实链结果后，才能标记 Sepolia 验收完成。

历史本地完整实跑 `run_id=main-full-63-1b7c9e` 发生在**延迟 materialize 修复前**：当时 63/63 完成、63/63 响应符合预期、0 `INFRA_ERROR`、63/63 锚定反向验证通过，并记录了 133 笔 Lineage 业务交易与 133 个事件、26,144,693 Lineage Gas、1,448,730 审计锚定 Gas 和约 188.05 秒运行时间。该 `133` 仅是旧实现的历史实测值，不能作为当前期望。

延迟 materialize 修复后的当前本地完整实跑 `run_id=sepolia-impl-local-final-a31f6d` 已复核：63/63 `COMPLETED`、0 `INFRA_ERROR`、63 个 anchor；15 个上链 Lineage 实例实际产生 97 笔 Lineage 交易/事件，另有 4 笔 DID setup，合计 164 笔交易。Lineage Gas 为 19,049,662，anchor Gas 为 1,448,790，DID setup Gas 为 204,888，总 Gas 为 20,703,340，运行约 203.5 秒。101 笔 DID+Lineage canonical 交易与 63 个 anchor 反查全部通过；A01–A06 的 Lineage 实例全部为 `prepared=true`、`materialized=false`、`enforcement=false`、0 Registry 交易。
...
6. 每项实验必须由独立子进程运行，并拥有独立 nonce、VC ID、VP、ReplayGuard、上下文、请求哈希、Lineage 子密钥、credential JTI、epoch ID 和预算 ID；这些标识在 63 项间保持隔离，但只有 H00、L01–L14 的 15 组 epoch/budget 会在 Baseline 通过后 materialize 到 Registry。
...
> 状态：核心协议、三个显式 Adapter、Hardhat/Sepolia DID 严格解析和 Sepolia 全量编排路径已实现并有自动化测试覆盖；在线入口接线及步骤 039 的完整门槛仍需单独复核。这里的“Sepolia 已实现”只表示执行路径可用，不表示已经付费完成真实 63 项远程验收。
...
- **本步只修改**：_experiments/security_comparison/adapters.py。
- **具体改动**：严格执行链下 Lineage `prepare` → shared protocol → BaselinePolicy → 链上 `materialize` → LineageGateway/LineageVerifier；`prepare` 只能构造不产生链状态的计划与签名材料，只有前两层全部通过后才能注册委托、epoch 和预算；随后校验委托链、权限收缩、预算、重放、四类撤销，并收集合约交易和事件引用。
- **验证命令**：

      conda run -n agentdid python -B -m unittest -v _experiments.security_comparison.tests.test_lineage_adapter

- **预期结果**：H00 通过；A01–A03 在共享协议层停止，A04–A06 在 Baseline 层停止，六项都不执行 `materialize` 且不发送 Registry 交易；合法低层材料上的 L01–L14 Lineage 违规在 lineage-agentdid 层拒绝。
- **完成条件**：decision trace 能证明三个验证层的调用顺序；chain-activity 能证明只有 H00 与 L01–L14 共 15 个 Lineage 实例占用 epoch 和预算命名空间。
...
- **具体改动**：为 A01-A03 断言三方案首个失败层为共享协议；A04-A06 断言共享协议通过且 Baseline/Lineage 在 Baseline 层失败；同时断言六个 A 类 Lineage 实例只完成链下 `prepare`，Registry 发送计数为零；L01-L14 断言共享协议与 Baseline 全通过后才允许 `materialize`。
- **验证命令**：

      conda run -n agentdid python -B -m unittest -v _experiments.security_comparison.tests.test_adapter_layering

- **预期结果**：所有非目标验证层均按设计通过，不出现“鲁棒性变体被错误低层提前拒绝”；完整矩阵只有 15 个 Lineage 实例产生 Registry 活动。
- **完成条件**：测试失败时输出 case、scheme、预期层、实际层、首个错误码、是否 materialized 和 Registry 发送计数。
...
- **预期结果**：Original、Baseline 接受，Lineage 以 `PARENT_MISMATCH` 拒绝；Baseline 通过后的链上 materialize/设置阶段产生 10 笔 Lineage 交易和 10 个事件，不开始预算调用。
...
- **预期结果**：Original、Baseline 接受，Lineage 从链上状态发现祖先撤销并以 `STATUS_REVOKED` 拒绝；Baseline 通过后的链上 materialize/设置阶段产生 7 笔 Lineage 交易和 7 个事件。
...
- **具体改动**：父进程把 ExperimentConfig 写入 exp_result/run_id/inputs；子进程只从一个明确 JSON 路径读取；校验 schema_version、run_id、scheme、case_id 与输出目录边界。
- **验证命令**：
...
- **具体改动**：实现 exp_result/run_id/experiments/scheme-directory/case_id；使用允许列表创建 12 个固定文件；拒绝 scheme、case 或 run_id 中的路径穿越字符。
- **验证命令**：
...
> 实现状态：本阶段的全量执行路径已经接线。父进程只执行一次完整只读预检；预检通过后最多执行 4 笔共享 DID 初始化，再按固定顺序串行运行 63 个子实验。Lineage 在链下 `prepare` 后延迟到共享协议和 Baseline 全通过才 `materialize`，所以 A01–A06 不发送 Registry 交易；实际上链的 Lineage 实例为 H00 与 L01–L14 共 15 个。当前计划总交易数为 160–164（97 笔 Lineage、63 笔审计锚定、0–4 笔 DID 初始化）。修复后的本地完整 run `sepolia-impl-local-final-a31f6d` 已按 164 笔交易通过验证；目前仍没有一次真实付费的 Sepolia 63 项结果，因此“本地全量已验证”与步骤 111 的“Sepolia 实链验收完成”必须分开记录。
...
- **具体改动**：由父进程探测 RPC 连通性、最新区块、chain ID 11155111、节点同步状态、费用报价与调用超时；只生成一次结构化 `preflight.json`；URL、Token、私钥与错误日志中的敏感值必须脱敏。
- **验证命令**：

      conda run -n agentdid python -B -m unittest -v _experiments.security_comparison.test_sepolia_full_preflight

- **预期结果**：正确模拟节点通过；错误 chain ID、无响应和未同步节点稳定失败；63 个子进程不重复执行远程完整预检。
- **完成条件**：预检失败时 planned=63、started=0，且不调用共享 DID 初始化或任何发送交易函数。
...
- **本步只修改**：preflight.py。
- **具体改动**：合并本地密钥配置：`config/agents_4_key.json` 提供完整 Agent admin/op 角色，`config/key.json` 覆盖 RPC 与 `issuer`，并将该 issuer 作为默认 relayer；可选的 `AGENTDID_EXPERIMENT_CHAIN_KEY` 具有最高优先级。随后只从私钥推导公共地址，逐账户检查 `latest nonce == pending nonce` 和余额；按 97 笔 Lineage、63 次 anchor、最多 4 次 DID 初始化估算 160–164 笔交易的安全上界。预检用 `AGENTDID_FULL_FEE_SAFETY_BPS` 将 RPC 报价扩大成强制 fee cap，并支持 `AGENTDID_FULL_MAX_COST_ETH` 总成本上限；Relayer 使用 `50,000,000` Gas 总预算，每个待初始化 DID controller 使用 `100,000` Gas 预算；私钥只在内存使用。
- **验证命令**：

      conda run -n agentdid python -B -m unittest -v _experiments.security_comparison.test_sepolia_full_preflight

- **预期结果**：所有付款账户 nonce 空闲且余额充足时通过；pending nonce、任一账户余额不足或估算成本超过用户上限时，在发交易前给出稳定错误码并停止。报告中的 Lineage/anchor/DID 单笔 Gas 上限分别为 `450,000`、`70,000`、`100,000`。
- **完成条件**：报告只出现公共地址、nonce、余额、Gas 单元、fee cap 和费用估算，不出现私钥、助记词或 RPC Token；余额需求以 Relayer 50,000,000 Gas 和各 controller 100,000 Gas 分账户计算。
...
- **具体改动**：检查两个地址为 checksum 非零地址且存在字节码，记录代码摘要；调用 DID Registry 的 `identityOwner`、`changed` 和 Lineage Registry 的 `domainSeparator`、`roots`、`budgets` 等只读接口；确认活跃 Root governance 与 relayer 一致、epoch 容量至少覆盖 15 个实际上链的 Lineage 实验，并确认本次 run 为 H00、L01–L14 派生的 15 个预算命名空间均未被占用。A01–A06 只有链下 `prepare`，不能预占 epoch 或预算命名空间。
- **验证命令**：

      conda run -n agentdid python -B -m unittest -v _experiments.security_comparison.test_sepolia_full_preflight

- **预期结果**：正确 Registry、治理和全新预算命名空间通过；EOA、空地址、错误 ABI、错误 domain、治理不匹配、epoch 不足或命名空间冲突均失败。
- **完成条件**：预检报告明确列出两个 Registry 的地址、代码摘要、接口、治理、epoch 和 15 个预算命名空间检查结果。
...
- **具体改动**：`main.py single/all --chain sepolia` 一旦选中，所有 DID 解析、注册、Lineage 交易和锚定均只能使用 Sepolia ChainConfig；删除或拒绝 localhost fallback；在每个证据文件记录 chain ID。
- **验证命令**：

      conda run -n agentdid python -B -m unittest -v _experiments.security_comparison.test_run_all _experiments.security_comparison.test_sepolia_did_resolver
...
- **具体改动**：完整只读预检通过后，父进程才确认或一次性设置 Issuer、Holder、Verifier 和其他控制 DID/delegate，最多产生 4 笔共享初始化交易；记录可复用交易引用；63 个实验仍派生独立 VC、VP、nonce、Lineage 子密钥、JTI、epoch ID 和 budget ID，但只有 H00、L01–L14 的 15 组 epoch/budget 在 Baseline 通过后 materialize 到 Registry。
- **验证命令**：

      conda run -n agentdid python -B -m unittest -v _experiments.security_comparison.test_run_all

- **预期结果**：共享控制设置只执行必要次数，63 个实验隔离字段仍全部唯一；链上只出现 15 组 Lineage epoch/budget，A01–A06 的 Registry 发送计数为零。
- **完成条件**：共享项、63 组链下独立标识和 15 组实际链上 Lineage 状态在代码模型及证据中有明确列表，不能混淆。
...
- **具体改动**：`main.py all --chain sepolia` 强制预检通过后生成完整 63 配置；拒绝与 `--scheme`、`--case`、`--sample` 或过滤参数组合；记录 planned=63；父进程将 `preflight.json` 路径与 SHA-256 完整性摘要传给子进程，子进程校验固定计划、run、experiment、chain、Actor、Registry 实时代码和摘要后才执行；该摘要是父子进程间防篡改绑定而不是面向不可信调用者的数字签名；共享交易账户固定串行，首个基础设施错误或链配置不一致时强制停止。
- **验证命令**：

      conda run -n agentdid python -B -m unittest -v _experiments.security_comparison.test_run_all _experiments.security_comparison.test_sepolia_full_preflight

- **预期结果**：Sepolia 正式模式只能计划并串行运行 63 项；任何抽样组合、伪造预检摘要或混用 Hardhat 配置都会在发交易前失败。
- **完成条件**：不存在名为 smoke 的结果被汇总成正式 Sepolia 安全结论，也不存在预检失败后启动本地链或继续下一个远程子实验的路径。

---

### 步骤 094A：在广播前强制执行 fee 与 Gas 上限

- **学习目标**：理解预检时“余额足够”不能替代每笔交易发送前的确定性费用边界。
- **本步只修改**：preflight.py、chain.py、infrastructure/lineage/registry_client.py 与 infrastructure/evidence_anchor.py。
- **具体改动**：父进程把预检得到的 `fee_upper_bound_wei` 作为所有发送路径的 gas price/EIP-1559 max fee 上限传给子进程；Lineage 估算 Gas 超过 `450,000`、DID 初始化估算 Gas 超过 `100,000` 时，在签名和广播前失败，anchor 发送器则固定构造 `70,000` Gas 的交易且不接受其他上限。完整预检同时用 Relayer `50,000,000` Gas 总预算及各 DID controller `100,000` Gas 预算核算余额与总成本上限。
- **验证命令**：

      conda run -n agentdid python -B -m unittest -v _experiments.security_comparison.test_sepolia_full_preflight _experiments.security_comparison.test_run_all

- **预期结果**：费用报价超过 fee cap，或任一发送路径超过相应 Gas 上限时，发送函数调用计数保持为零并返回稳定错误码。
- **完成条件**：不存在先广播再通过回执 Gas 判断“超限”的路径；所有超限都必须在 raw transaction 广播前终止。
...
- **本步只修改**：默认不修改代码；若实链运行暴露 Sepolia 环境特有问题，只做针对性修复，禁止增加回退或减少 case。
- **具体改动**：由用户显式准备环境变量和充足 Sepolia 测试币，设置可接受的成本上限及全新 `run-id`；执行完整命令；先审阅父进程一次性预检报告，再保留全部 63 项链上回执。出现 `INFRA_ERROR` 时结果不通过，修复后必须另开新 `run-id` 重跑，不能把部分 run 拼接成验收结果。
- **验证命令**：

      $env:AGENTDID_FULL_MAX_COST_ETH = "<用户接受的成本上限>"
      conda run -n agentdid python -B main.py all --chain sepolia --run-id <全新 run-id> --timeout-seconds 900

- **预期结果**：父进程预检通过后串行执行完整 63 项，整次运行约产生 160–164 笔测试网交易，其中 97 笔来自 15 个 materialized Lineage 实例，并且恰好有 63 个 Sepolia 审计 anchor；安全向量与本地一致。
- **完成条件**：63 项 COMPLETED、0 INFRA_ERROR、63 个 anchor 均在 chain ID 11155111 上反向验证；否则不得声称 Sepolia 验收完成。

> 当前状态：全量 Sepolia 功能已实现，但本手册没有记录一次已经完成的真实付费远程运行。执行本步骤会真实发送大量测试网交易；只有用户明确启动并满足上述完成条件后，才可勾选相应实链验收项。
...
> 下列复选框是供每次发布或重新部署时逐项执行的可复用模板，默认保留未勾选；它不覆盖本文顶部“当前执行状态”及对应 run_id 已记录的事实。本次本地完整结果以 `sepolia-impl-local-final-a31f6d` 为准，Sepolia 付费实链验收项仍未完成。
...
- [ ] Sepolia 父进程只执行一次完整预检，并通过 RPC、同步状态、chain ID、两个 Registry 字节码/接口检查。
- [ ] Sepolia 预检通过 Root governance、15 个 Lineage epoch 容量、15 个预算命名空间、各付款账户 latest/pending nonce、余额和成本上限检查。
- [ ] A01–A06 的 Lineage 实例仅做链下 prepare；共享协议或 Baseline 通过前不 materialize，且 Registry 发送计数为零。
- [ ] 预检 fee cap、Lineage 450,000 Gas、anchor 70,000 Gas、DID controller 100,000 Gas 与 Relayer 50,000,000 Gas 总预算均在广播前强制执行。
- [ ] Sepolia 63 个子实验串行执行，并分别验证父进程预检报告的路径、摘要、run、experiment 与 chain ID 绑定。
- [ ] Sepolia 模式没有本地回退，也没有抽样执行；首个基础设施错误后强制停止。
- [ ] 用户已显式完成一次真实付费 Sepolia 全量运行，并取得 63/63 COMPLETED、0 INFRA_ERROR、63 个可反向验证 anchor；在此之前本项保持未勾选。
